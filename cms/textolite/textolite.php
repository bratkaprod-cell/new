...
final class d
{
    ...
    private function m()
    {
        $a = function_exists('random_bytes') ? bin2hex(random_bytes(20)) : sha1(time() . mt_rand());
        $this->a->j(o_, $a);
    ...
    {
        $b = $this->a->i(B_);
        if (!$b || preg_match('~(?:^|,\s*)' . preg_quote($a, '~') . '(?:\s*,|$)~i', $b)) return true;
    }
